# Unprivileged User Installation {#nosudo}

DataRobot requires escalated privileges to install dependency packages, configure Docker, and configure logging on your system.
...
Once these steps are performed, your cluster will be ready to run the DataRobot installation process.
...
## Configure Docker
Configure Docker to use the Docker registry that will run on the Edge Node and use the syslog driver for container logs.
...
If it isn’t, or you would like to use a different group name, run the following on all nodes with your choices of group name and DataRobot user name:
```bash
    sudo groupadd docker
    sudo usermod -a -G docker druser
    sudo service docker restart
```
Verify that your DataRobot user can access the Docker daemon on all nodes by logging in as your DataRobot user and running:
```bash
    sudo su druser
    docker info
```
...
## Docker-py
The `docker-py` Python package, used by the installer to issue commands to Docker, must be present on all application servers.

* Copy the rpm files in `release/docker-packages/docker-py-packages/rpm/` to all application servers.
...
* Add the `docker-py` libraries to site-packages so they are accessible (**NOTE**: `cp` may print an error about backports directory.
...
## /etc/hosts File
DataRobot services on the edge node use `/etc/hosts` entries to identify the locations of some services.
Edit your `/etc/hosts` file to append the following lines, with IP addresses replaced as appropriate:
```
...
Substitute `LOG_SERVER` with the IP address or hostname of the server chosen to be the central logging server:
...
Review your system’s RSYSLOG configuration (`/etc/rsyslog.conf` and `/etc/rsyslog.conf.d/`) to see if these messages will end up in any other log file, such as `/var/log/daemon.log` or `/var/log/messages`, and adjust your RSYSLOG and logrotate configuration to handle these messages.

DataRobot does not configure your system log rotation or RSYSLOG for you.
An example logrotate configuration is below.
...
## Crontab
DataRobot servers cache temporary files to speed up certain operations, necessitating a cleanup script to run regularly to remove old files from the cache.

In the DataRobot or root user’s crontab, put the following line to regularly clean up old files.
```
