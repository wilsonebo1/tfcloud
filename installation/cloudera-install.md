...
```bash
scp ~/hadoop/DataRobot-5.1.x*.{jar,parcel,parcel.sha} \
...
```bash
sudo mv /tmp/DataRobot-5.1.x/*.jar /opt/cloudera/csd/
```
...
sudo chown cloudera-scm:cloudera-scm \
    /opt/cloudera/csd/DataRobot-5.1.x*.jar
```
...
```bash
sudo mv /tmp/DataRobot-5.1.x*.parcel /opt/cloudera/parcel-repo
```

* Move the parcel file's SHA:

```bash
sudo mv /tmp/DataRobot-5.1.x*.parcel.sha /opt/cloudera/parcel-repo
```
...
```
# FILE: DataRobot-5.1.x-release-el6.parcel.sha
80e5223337d8978432ccae99ffea55f92e4fb4b9 DataRobot-5.1.x-release-el6.parcel
```
...
```bash
cat /tmp/DataRobot-5.1.x-release.el6.parcel | cut -d ' ' -f 1 \
| sudo tee /opt/cloudera/parcel-repo/DataRobot-5.1.x-release.el6.parcel
```
